...
const DepthText = ({
  ...
  // If depthColor is not provided, the theme's gradientPrimary is used.
  depthColor,
  ...
  // Use the manually supplied depthColor if one exists.
  // Otherwise, use the current theme's gradientPrimary.
  const resolvedDepthColor = depthColor ?? theme.colors.gradientPrimary;

  // Likewise, use the theme's fontPrimary unless faceColor
  // has been explicitly supplied.
  const resolvedFaceColor = faceColor ?? theme.colors.fontPrimary;
  ...
  const depthLayers = useMemo(
    ...
    [safeLayers, safeDepth, resolvedFaceColor, resolvedDepthColor],
  );
  ...
  const rootStyle = {
    ...
    "--depth-text-face-color": resolvedFaceColor,

    "--depth-text-depth-color": resolvedDepthColor,

    "--depth-text-shadow": shadow
      ? `0 22px 34px color-mix(in srgb, ${resolvedDepthColor} 36%, transparent), 0 4px 8px rgba(0, 0, 0, 0.28)`
      : "none",
  };
